...
const DEFAULT_MANIFEST = path.join(ROOT, "tests/fixtures/ported-app-tests.json");
...
const PARENT_PREFIXES = { "control-plane": "W", github: "G", collector: "C", runner: "R" };
...
export function extractTopLevelTests(source, sourceFile = "suite") {
  ...
  return starts.map((match, index) => {
    ...
    if (callEnd < match[0].length) fail(`malformed top-level test call in ${sourceFile} at ordinal ${index + 1}`);
    let name;
    try { name = JSON.parse(match[1]); } catch { fail(`invalid test name string in ${sourceFile} at ordinal ${index + 1}`); }
...
export function validatePortManifest(manifest) {
  if (manifest?.schema_version !== 1 || !Array.isArray(manifest.tests)) fail("invalid ported-test manifest");
  if (manifest.source_commit !== SOURCE_COMMIT || manifest.frozen_app_test_count !== 104
    || manifest.required_current_test_count !== 105 || manifest.tests.length !== 105) fail("manifest baseline annotation or source commit drifted");
  if (manifest.hash_contract !== HASH_CONTRACT) fail("manifest hash contract drifted");
  if (JSON.stringify(manifest.post_baseline_security_deltas) !== JSON.stringify(["immutable-control-ref"])) fail("manifest security delta drifted");
  const ids = new Set();
  let offset = 0;
  for (const [prefix, count] of Object.entries(PREFIXES)) {
    for (let ordinal = 1; ordinal <= count; ordinal += 1) {
      ...
      if (record?.id !== id || ids.has(id)) fail(`manifest identity/order drift at ${id}`);
      ids.add(id);
      if (record.source_commit !== SOURCE_COMMIT || record.source_file !== SOURCE_FILES[prefix]
        || record.source_ordinal !== ordinal || record.target_suite !== TARGET_SUITES[prefix]) fail(`manifest source or mapping drift at ${id}`);
      if (record.responsibility_group !== expectedGroup(id)) fail(`manifest group drift at ${id}`);
      if (typeof record.name !== "string" || !record.name || !digest(record.body_sha256)) fail(`invalid manifest record ${id}`);
      if (APPROVED_DELTAS.has(id)) {
        if (record.assertion_mode !== "approved_delta" || typeof record.approved_contract_delta_reason !== "string"
          || record.approved_contract_delta_reason !== DELTA_REASONS.get(id) || !("approved_target_body_sha256" in record)
          || (record.approved_target_body_sha256 !== null && !digest(record.approved_target_body_sha256))) fail(`invalid approved delta ${id}`);
      } else if (record.assertion_mode !== "mechanical" || "approved_contract_delta_reason" in record
        || "approved_target_body_sha256" in record) fail(`invalid mechanical manifest record ${id}`);
    }
  }
  if (manifest.tests.find((record) => record.id === "W29")?.assertion_mode !== "mechanical") fail("W29 immutable-ref classification must remain mechanical");
  return manifest;
}

export function verifyObservedRecord(record, observed) {
  if (observed.name !== record.name) fail(`renamed target identity ${record.id}: ${observed.name}`);
  if (record.assertion_mode === "mechanical") {
    if (observed.body_sha256 !== record.body_sha256) fail(`changed body without approved delta ${record.id}`);
...
export function selectGroup(manifest, group = null) {
  ...
  if (PARENT_PREFIXES[group]) return { records: valid.tests.filter((record) => record.id.startsWith(PARENT_PREFIXES[group])), strict: false, group };
  const records = valid.tests.filter((record) => record.responsibility_group === group);
...
export function verifySuiteRecords(records, source, { strict = true } = {}) {
  ...
  for (const observed of actual) {
    if (names.has(observed.name)) fail(`duplicate target identity ${observed.name}`);
    names.add(observed.name);
  }
  const expectedByName = new Map(records.map((record) => [record.name, record]));
  const seen = new Set();
  for (let index = 0; index < actual.length; index += 1) {
    ...
    if (!record) {
      ...
      if (positional && !seen.has(positional.id)) fail(`renamed target identity ${positional.id}: ${observed.name}`);
  ...
  return { ids: records.filter((record) => seen.has(record.id)).map((record) => record.id) };
}

function contained(root, relative) {
  const resolvedRoot = path.resolve(root);
  const resolved = path.resolve(resolvedRoot, relative);
  if (!resolved.startsWith(`${resolvedRoot}${path.sep}`)) fail(`source file escapes source root: ${relative}`);
  return resolved;
}
...
export function recomputeSourceIdentities(records, sourceRoot) {
  ...
  for (const [relative, expected] of byFile) {
    const actual = extractTopLevelTests(fs.readFileSync(contained(sourceRoot, relative), "utf8"), relative);
    const ordinals = expected.map((record) => record.source_ordinal).sort((a, b) => a - b);
    if (ordinals[0] === 1 && ordinals.every((ordinal, index) => ordinal === index + 1) && actual.length !== expected.length) {
      fail(`source identity count drift in ${relative}`);
    }
    for (const record of expected) {
      const observed = actual[record.source_ordinal - 1];
      if (!observed || observed.name !== record.name || observed.body_sha256 !== record.body_sha256) fail(`source identity drift at ${record.id}`);
      ids.push(record.id);
    }
  }
  return { ids };
...
function readManifest(file) {
  try { return validatePortManifest(JSON.parse(fs.readFileSync(file, "utf8"))); }
  catch (error) { fail(`cannot read manifest: ${error.message}`); }
}

function parseArgs(argv) {
  const args = { manifest: DEFAULT_MANIFEST, suiteRoot: ROOT, group: null, sourceRoot: null };
  for (let index = 0; index < argv.length; index += 2) {
    const flag = argv[index]; const value = argv[index + 1];
    if (!["--manifest", "--suite-root", "--group", "--source-root"].includes(flag) || !value) fail("usage: verify-ported-tests.mjs [--manifest ABS] [--suite-root ABS] [--group GROUP] [--source-root ABS]");
  ...
  if (!path.isAbsolute(args.manifest) || !path.isAbsolute(args.suiteRoot) || (args.sourceRoot && !path.isAbsolute(args.sourceRoot))) fail("manifest, suite root, and source root paths must be absolute");
  return args;
}
...
export function verify({ manifestPath, suiteRoot, group = null }) {
  ...
  for (const record of selection.records) {
    ...
    list.push(record); suites.set(record.target_suite, list);
  }
  const ids = [];
  for (const [suite, records] of suites) {
    ...
    const allSuiteRecords = manifest.tests.filter((record) => record.target_suite === suite);
    const allByName = new Map(allSuiteRecords.map((record) => [record.name, record]));
    const selectedIds = new Set(records.map((record) => record.id));
    const seenNames = new Set();
    const observedIds = new Set();
    for (const observed of actual) {
      if (seenNames.has(observed.name)) fail(`duplicate target identity ${observed.name} in ${suite}`);
      ...
      if (selectedIds.has(record.id)) { verifyObservedRecord(record, observed); observedIds.add(record.id); }
    }
    if (selection.strict) {
      const missing = records.find((record) => !observedIds.has(record.id));
      if (missing) fail(`missing target identity ${missing.id}`);
    }
    ids.push(...records.filter((record) => observedIds.has(record.id)).map((record) => record.id));
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    ...
    const result = args.sourceRoot ? recomputeSourceIdentities(manifest.tests, args.sourceRoot)
      : verify({ manifestPath: args.manifest, suiteRoot: args.suiteRoot, group: args.group });
    process.stdout.write(`verified ${result.ids.length} identities ids=${result.ids.join(",")}\n`);
  } catch (error) {
