...
const HASH_ONE =
  "7381da16b1b7c54c198e11fcf1dc231834199c0f2886cc11f03cee3a7cb1657d";
const HASH_TWO =
  "00750542256670cd020ab09d429d7438840d1a48eeaf6af55bfffac863614542";
const W05_TARGET_BODY_SHA256 =
  "40ec0a369daaf17f681528d3523a45108612031a52a031fe95652c1e202a88bc";
const W06_TARGET_BODY_SHA256 =
  "1bcd87d92047499d06d21abe741a63089a859908e2f8067d97db4331be6017fe";
const W37_TARGET_BODY_SHA256 =
  "a78e0618be1dfec6ed55158eac1dac2170c0a0029e008796101b668245bfdec8";
const W38_TARGET_BODY_SHA256 =
  "e84702fd12b3309dc2a16c6d485627ee929cd23cef1a6ff87de3be4e351a7cac";
const BOUND_TARGET_BODIES = new Map([
  ["W05", W05_TARGET_BODY_SHA256],
  ["W06", W06_TARGET_BODY_SHA256],
  ["W37", W37_TARGET_BODY_SHA256],
  ["W38", W38_TARGET_BODY_SHA256],
]);
...
function expectedMapping(id) {
  return {
    ...
    C: [
      ...
      "tests/unit/target-collector.test.mjs",
    ],
...
test("committed manifest has exact mappings, hierarchical groups, and plan-approved pending deltas", () => {
  ...
  for (const record of value.tests) {
    ...
    assert.equal(
      record.responsibility_group,
      expectedGroup(record.id),
      record.id,
    );
    if (APPROVED_DELTAS.has(record.id)) {
      ...
      assert.equal(
        ...
        BOUND_TARGET_BODIES.get(record.id) ?? null,
        record.id,
      );
      assert.equal(
        record.approved_contract_delta_reason,
        DELTA_REASONS.get(record.id),
        record.id,
      );
    } else {
  ...
  assert.equal(
    value.tests.find((record) => record.id === "W29").assertion_mode,
    "mechanical",
  );
  assert.deepEqual(value.post_baseline_security_deltas, [
    "immutable-control-ref",
  ]);
});
...
test("manifest validator rejects source, mapping, group, and delta corruption", async (t) => {
  const cases = [
    [
      "source commit",
      (value) => {
        value.tests[0].source_commit = "f".repeat(40);
      },
    ],
    [
      "source file",
      (value) => {
        value.tests[0].source_file = "tests/wrong.test.mjs";
      },
    ],
    [
      "target suite",
      (value) => {
        value.tests[0].target_suite = "tests/unit/wrong.test.mjs";
      },
    ],
    [
      "group",
      (value) => {
        value.tests[0].responsibility_group = "wrong";
      },
    ],
    [
      "pending delta field",
      (value) => {
        delete value.tests.find((record) => record.id === "W01")
          .approved_target_body_sha256;
      },
    ],
    [
      "bound delta digest",
      (value) => {
        value.tests.find(
          (record) => record.id === "W01",
        ).approved_target_body_sha256 = "not-a-digest";
      },
    ],
    [
      "mechanical delta metadata",
      (value) => {
        value.tests.find(
          (record) => record.id === "W29",
        ).approved_target_body_sha256 = null;
      },
    ],
    [
      "immutable-ref classification",
      (value) => {
        value.tests.find((record) => record.id === "W29").assertion_mode =
          "approved_delta";
      },
    ...
    [
      "hash contract",
      (value) => {
        value.hash_contract = "generic SHA-256";
      },
    ],
    [
      "generic delta reason",
      (value) => {
        value.tests.find(
          (record) => record.id === "W01",
        ).approved_contract_delta_reason =
          "Approved standalone adaptation required by the frozen design contract.";
      },
    ],
    [
      "swapped delta reason",
      (value) => {
  ...
  for (const [name, mutate] of cases) {
    await t.test(name, () => {
      ...
      mutate(value);
      assert.throws(
        () => verifier.validatePortManifest(value),
        /manifest|mapping|group|delta|mechanical|W29/i,
      );
    });
  }
});
...
test("pending approved deltas reject a target until bound, then freeze the approved target body", () => {
  const record = structuredClone(
    manifest().tests.find((item) => item.id === "W01"),
  );
  const observed = { name: record.name, body_sha256: HASH_ONE };
  assert.throws(
    () => verifier.verifyObservedRecord(record, observed),
    /pending approved target body/i,
  );
  record.approved_target_body_sha256 = HASH_ONE;
  assert.doesNotThrow(() => verifier.verifyObservedRecord(record, observed));
  assert.throws(
    () =>
      verifier.verifyObservedRecord(record, {
        ...observed,
        body_sha256: HASH_TWO,
      }),
    /approved target body drift/i,
  );
});

test("mechanical records require source body equality", () => {
  const record = structuredClone(
    manifest().tests.find((item) => item.id === "W29"),
  );
  assert.doesNotThrow(() =>
    verifier.verifyObservedRecord(record, {
      name: record.name,
...
test("parent groups allow cumulative partial observation while leaf groups require every assigned identity", () => {
  ...
  assert.deepEqual(
    parent.records.map((record) => record.id),
    value.tests
      .filter((record) => record.id.startsWith("W"))
      .map((record) => record.id),
  );
  const leaf = verifier.selectGroup(value, "control-plane/webhook-boundary");
  assert.equal(leaf.strict, true);
  assert.deepEqual(
    leaf.records.map((record) => record.id),
    ["W05", "W06", "W38"],
  );
  assert.deepEqual(
    verifier
      .selectGroup(value, "control-plane/webhook-lifecycle")
      .records.map((record) => record.id),
    ["W03", "W04"],
  );
  assert.deepEqual(
    verifier
      .selectGroup(value, "control-plane/installation-authority")
      .records.map((record) => record.id),
    ["W09", "W10"],
  );
  assert.equal(
    verifier
      .selectGroup(value, "control-plane/callback")
      .records.some((record) => record.id === "W07"),
    true,
  );
  const workflow = verifier.selectGroup(value, "runner");
  assert.equal(
    workflow.records.some((record) => record.id === "R21"),
    true,
  );
});
...
function suite(
  firstName = "first behavior",
  secondName = "second behavior",
  secondAssertion = "assert.equal(2, 2);",
) {
...
test("suite comparison fails closed for missing, renamed, duplicate, changed, and extra identities", async (t) => {
  ...
  const cases = [
    ...
    [
      "changed",
      suite("first behavior", "second behavior", "assert.equal(1, 2);"),
      /changed body.*W02/i,
    ],
    [
      "extra",
  ...
  for (const [name, source, pattern] of cases) {
    await t.test(name, () => {
      assert.throws(
        () => verifier.verifySuiteRecords(records, source, { strict: true }),
        pattern,
      );
    });
  }
...
test("controller source-root recomputation checks supplied roots without a default donor dependency", () => {
  const sourceRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "ported-source-root-"),
  );
  ...
  try {
    ...
    assert.throws(
      () => verifier.recomputeSourceIdentities(records, sourceRoot),
      /source identity.*W01/i,
    );
  } finally {
